...
const ShippingPage = () => {
  ...
  // Load cart details (Book config + price pulled from localStorage)
  useEffect(() => {
    const loadCartDetails = async () => {
      setIsLoading(true);
      setError(null);
      try {
        // Example: stored book configuration in localStorage
  ...
  // Load box dimensions for shipping from localStorage
  useEffect(() => {
  ...
  // Load list of countries from "country-list"
  useEffect(() => {
    setCountries(getData());
  }, []);

  // Recalculate discounts/taxes/total when region or shippingCost changes
  useEffect(() => {
    if (!cart) return;
  ...
  // Utility to render book customization summary
  const renderCustomizations = () => {
    if (cart && cart.items && cart.items[0]?.configuration) {
      ...
      if (config.pageCount) parts.push(`Pages: ${config.pageCount}`);
      ...
      if (config.activeOption === 'thesis-binding') {
        ...
        if (config.thesisExteriorColor)
          parts.push(`Exterior: ${formatOptionLabel(config.thesisExteriorColor)}`);
        if (config.thesisFoilStamping)
          parts.push(`Foil: ${formatOptionLabel(config.thesisFoilStamping)}`);
        if (config.thesisScreenStamping)
          parts.push(`Screen: ${formatOptionLabel(config.thesisScreenStamping)}`);
        if (config.thesisCornerProtector)
          parts.push(`Corners: ${formatOptionLabel(config.thesisCornerProtector)}`);
      }
  ...
  // Fetch FedEx rate from Easyship
  const fetchFedexRate = async () => {
    setFedexError(null);
    setIsFetchingFedex(true);

    try {
      if (!country || !region || !city || !postalCode) {
        setFedexError(<p className="error-message">Please fill in country, state, city, and postal code first.</p>);
        return;
      }
      ...
      const taxes = region === 'Texas' ? parseFloat(((subtotal - discountAmount) * 0.0825).toFixed(2)) : 0;

      const shippingVal = parseFloat(amount.toFixed(2));
      const newTotal = parseFloat(((subtotal - discountAmount + shippingVal + taxes)).toFixed(2));
  ...
  // Handle form submission: fetch FedEx rate → submit shipping info → navigate
  const handleSubmitShipping = async (e) => {
    ...
    try {
      // 1) Fetch FedEx rate and update cart (including localStorage)
      await fetchFedexRate();

      // 2) Now that cart has updated shippingCost and total, submit shipping info
      const shippingData = {
        firstName,
        lastName,
      ...
      // Cart already updated in localStorage by fetchFedexRate()
      navigate('/checkout/payment');
    } catch (err) {
      console.error('Shipping info submission failed:', err);
      // If fetchFedexRate threw an error, fedexError is already set, but we can also set a generic error
      setError(err.response?.data?.message || err.message || 'Failed to submit shipping information.');
    } finally {
